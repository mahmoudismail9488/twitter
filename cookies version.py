...
keys=[]
ids_num=[]
with open("./my files/keys and APIs/keys.txt","r",encoding="utf-8") as keys_file:
    my_keys=keys_file.readlines()
    for i in my_keys:
        keys.append(i.split(":")) 
#drop down list to get the id depend on the number of keys   
fisrt_id=1
while fisrt_id <= len(keys):
    ids_num.append(fisrt_id)
    fisrt_id=fisrt_id+1
...
drop = OptionMenu( me , clicked , *ids_num )   
